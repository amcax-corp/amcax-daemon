...
- 您可以选择一些对象，使用 `导出选中...`功能将它们保存为文件；也可使用 `保存` 功能来保存工作空间中的所有对象。对象将以 `*.abr` 格式保存，这是 AMCAX 的专有格式。您可以稍后将对象从 `*.abr` 文件加载到工作空间中。
...
- 不能混用Debug/Release的内核库。因此，如果您获取到的是Release版本的内核库，目前暂时只能编译运行Release版本的九韶精灵。
