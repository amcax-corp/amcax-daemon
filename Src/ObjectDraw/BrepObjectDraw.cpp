...
{
	brep_object_ = dynamic_cast<BRepObject*> (object);
}
...
void acamcad::BRepObjectDraw::drawSelected(const SelectModel& s_model, int s_id) const
{
    if (s_model == SelectModel::VERTEX_MODEL)
    {
        ...
        const AMCAX::IndexSet<AMCAX::TopoShape>& vertexs = brep_object_->getShapeVertices();
...
{
    const AMCAX::TopoShape& shape = brep_object_->shape_;
    if (brep_object_->getShape().IsNull())
        return;
    if (brep_object_->getShapeVertices().size() == 0)
        return;
...
void acamcad::BRepObjectDraw::updatedrawState()
{
    ...
    AMCAX::BRepMeshIncrementalMesh IMesh(brep_object_->shape_, 0.005, true);
    AMCAX::MakeShapeTool::EnsureNormalConsistency(brep_object_->getShape());

    const AMCAX::IndexSet<AMCAX::TopoShape>& faces = brep_object_->getShapeFaces();
    ...
    const AMCAX::IndexSet<AMCAX::TopoShape>& edges = brep_object_->getShapeEdges();
...
void acamcad::BRepObjectDraw::draw(const DrawModel& dmodel, bool is_show_point) const
{
    ...
    bool is_selected = brep_object_->isSelect();
