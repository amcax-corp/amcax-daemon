...
void CreateOperate_Builder::operateWithBRep(BRepObject *brep_object)
{
    if (shapes.size())
    {
    ...
    {
        brep_object->getShape() = faces.back();
    }
    else if (wires.size())
    {
        brep_object->getShape() = wires.back();
    }
    else if (edges.size())
    {
        brep_object->getShape() = edges.back();
    }
    else
    {
    }
