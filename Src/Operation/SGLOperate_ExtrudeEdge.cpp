...
bool acamcad::SGLOperate_ExtrudeEdgeH::DoOperate(AdapterObject* adapter)
{
	...
	case DataType::MESH_TYPE:
	{
		...
		{
			std::vector<int> faces = AMCAX::SubD::MeshTool::EdgeFaceIndexs(adapter->mesh->mesh(),
				s_id_list[i]);
	...
	case DataType::TSPLINEU_TYPE:
	{
		for (int i = 0; i < s_id_list.size(); i++)
		{
			std::vector<int> faces = AMCAX::TMS::TMSplineTool::EdgeFaceIndexs(adapter->tSpline->getTSpline(),
				s_id_list[i]);
