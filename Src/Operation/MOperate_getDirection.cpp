...
void MOperation_getDirection::operateWithBRep(BRepObject* brep_object)
{
	...
	if (selmodel_ == SelectModel::EDGE_MODEL)
	{
		...
		const AMCAX::TopoEdge& e = static_cast<const AMCAX::TopoEdge&>(brep_object->getShapeEdges()[s_id]);
	...
	else if (selmodel_ == SelectModel::FACE_MODEL)
	{
		...
		const AMCAX::TopoFace& f = static_cast<const AMCAX::TopoFace&>(brep_object->getShapeFaces()[s_id]);
