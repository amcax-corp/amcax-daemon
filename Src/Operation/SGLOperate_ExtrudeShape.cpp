...
namespace acamcad
{
	...
	void acamcad::SGLOperate_ExtrudeShape::operateWithBRep(BRepObject* brep_object)
	{
		if (brep_object->getShapeFaces().size() != 1)
		{
			std::cout << "extrude shape with multi face";
			return;
		}

		AMCAX::TopoShape f = AMCAX::CopyShape(brep_object->getShapeFaces()[0]);

		AMCAX::MakePrism prim(f, AMCAX::Vector3(dire_.X(), dire_.Y(), dire_.Z()));
		brep_object->getShape() = prim.Shape();
	}
